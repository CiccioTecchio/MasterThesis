...
Cliccando sull'icona posta accanto alla barra dell'indirizzo su Google Chrome, viene visualizzato il popup di Knoxly. Accanto all'icona è possibile inoltre visualizzare un badge che riporta un contatore numerico (vedi Figura~\ref{}). Tale contatore è utilizzato per segnalare all'utente il numero di frasi scritte ritenut 

Alla UI di Knoxly si accede cliccando sulla sua icona posizionata nella sezione di Google Chrome dedicata ai plug-in. Sull'icona di Knoxly possiamo trovare un badge che riporta il numero di frasi contenenti dati sensibili e/o personali individuate.
\begin{figure}[h]
    \centering
...
In figura \ref{fig:uiKnoxly} viene mostrata la UI di Knoxly essa è composta da 3 sezioni la sezione \textbf{Analysis}è composta da una tabella da 3 colonne e mostra le frasi che Knoxly reputa sensibili, un badge che riporta una icona che fa riferimento al topic a cui appartiene la frase, il badge può essere colorato di giallo nel caso in cui la frase risulti sensibile(sensibilità positiva compresa fra 0.36 e 0.68) o di rosso nel caso in cui la frase abbia una sensibilità positiva maggiore o uguale a 0.68 e un bottone \textbf{feedback} che consente all'utente di inviare un feedback a Knoxly per far si che il tool non segnali più frasi simili a quella appena segnalata.

La sezione \textbf{Statistics}(figura \ref{fig:statKnoxly} contiene un grafico a barre che riporta il numero di occorrenza di frasi sensibili nei vari topic. Sotto al grafico a barre è presenta la rappresentazione dell'\textbf{overall score} esso viene calcolato in base alla media aritmetica delle sensibilità delle frasi considerate sensibili da Knoxly. L'overall score viene rappresentato come un cerchio, esso si colora di rosso se l'overall score supera 0.68 altrimenti si colora di giallo. 
\begin{figure}[h]
    \centering
